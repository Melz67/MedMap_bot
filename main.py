...
# 1. تحميل متغيرات البيئة (مهم جداً للتشغيل المحلي)
load_dotenv()

# 2. جلب التوكن من النظام (سيقرأه من Railway Variables)
...
# --- وظائف البوت (بدون تغيير) ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [["📊 إنشاء تقرير جديد"], ["✅ تسجيل زيارة جديدة"], ["📤 إرسال التقرير"]]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text("🤖 *Medical Rep Bot*\n\nمرحباً! اختر من القائمة:", reply_markup=reply_markup, parse_mode='Markdown')
    return MAIN_MENU

async def main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    choice = update.message.text
    if choice == "📊 إنشاء تقرير جديد":
        await update.message.reply_text("👤 أدخل اسمك (سيظهر في التقرير):", reply_markup=ReplyKeyboardRemove())
        return NAME_INPUT
    elif choice == "✅ تسجيل زيارة جديدة":
        keyboard = [["🌅 A.M Visit"], ["🌆 P.M Visit"], ["💊 Pharmacy Visit"], ["🔙 رجوع"]]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
async def name_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        await update.message.reply_text(f"✅ *تم إنشاء التقرير بنجاح!*\n\n📄 اسم الملف: `{filename}`\n👤 الاسم: {user_name}", parse_mode='Markdown')
    else:
        await update.message.reply_text(f"ℹ️ *التقرير موجود بالفعل*\n\n📄 اسم الملف: `{filename}`", parse_mode='Markdown')
...
async def visit_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "🌅 A.M Visit":
        ...
        await update.message.reply_text("🏪 أدخل اسم الصيدلية:", reply_markup=ReplyKeyboardRemove())
        return PHARMACY_NAME
    await update.message.reply_text("👤 أدخل اسم الدكتور:", reply_markup=ReplyKeyboardRemove())
    return DOCTOR_NAME
...
async def products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [["⏭️ تخطي"]]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text("💬 أدخل التعليق (أو اضغط تخطي):", reply_markup=reply_markup)
    return COMMENT
...
async def comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ *تم تسجيل الزيارة بنجاح!*\n\n📄 تم الحفظ في: `{os.path.basename(filepath)}`", reply_markup=ReplyKeyboardRemove(), parse_mode='Markdown')
...
async def pharmacy_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ *تم تسجيل زيارة الصيدلية بنجاح!*\n\n📄 تم الحفظ في: `{os.path.basename(filepath)}`", reply_markup=ReplyKeyboardRemove(), parse_mode='Markdown')
    context.user_data.clear()
    return await start(update, context)
...
async def send_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    filepath = os.path.join(REPORTS_DIR, filename)
    if not os.path.exists(filepath):
        await update.message.reply_text("⚠️ *لا يوجد تقرير لليوم!*\n\nقم بإنشاء تقرير جديد أولاً.", parse_mode='Markdown')
        return await start(update, context)
    waiting_msg = await update.message.reply_text("⏳ جاري إرسال التقرير...")
    try:
        with open(filepath, 'rb') as file:
            await update.message.reply_document(document=file, filename=filename, caption=f"📊 *تقرير اليوم*\n\n📅 {datetime.now().strftime('%d %B %Y')}", parse_mode='Markdown')
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("تم الإلغاء. استخدم /start للبدء مجدداً.", reply_markup=ReplyKeyboardRemove())
    context.user_data.clear()
    return ConversationHandler.END

# --- وظيفة التشغيل الرئيسية (معدلة) ---
def main():
    """تشغيل البوت باستخدام التوكن من متغيرات البيئة"""
    
    # التحقق من وجود التوكن لمنع توقف البوت فجأة
    if not BOT_TOKEN:
